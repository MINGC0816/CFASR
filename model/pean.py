...
class PEAN(nn.Module):
    ...
    def forward(self, x, rec_result):
        ...
        x_im = block['1'].contiguous().view(B, -1, H*W).permute(2, 0, 1) #重整特征图形状
        rec_result = rec_result.transpose(0, 1) #转置 rec_result 的前两维，将形状从 (L, B, C) 变为 (B, L, C)，方便后续处理。
        ...
        x_pos_mem = self.pe(torch.zeros((B, L, C)).to(self.device)).permute(1, 0, 2) #添加位置编码
        mask_mem = torch.zeros((B, L)).to(self.device).bool() #创建注意力掩码
        prior, _ = self.upsample_transformer(prior, mask_mem, self.init_factor.weight, x_pos_mem, x_im) #使用变压器更新先验特征
        prior = prior.mean(0) #平均池化先验特征
        prior = prior.permute(1, 2, 0).view(B, -1, H, W) #恢复原始特征图形状
