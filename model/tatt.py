...
if __name__ == "__main__":
    ...
    # 选择 mask=True or False
    model = TSRN_TL(mask=True).to(device)

    # 如果 mask=True 输入应该是 4 通道，否则是 3 通道
    in_ch = 4 if model.block1[0].in_channels == 4 else 3
    x = torch.randn(1, in_ch, 32, 128).to(device)
